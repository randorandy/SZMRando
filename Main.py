...
def write_location(romWriter: RomWriter, location: Location) -> None:
    ...
    for address in location['locids']:
        ...
        #if item[0] == "Space Jump" :
        #    romWriter.writeBytes(address+4, b"\x8c")
        #if item[0] == "Screw Attack" :
        #    romWriter.writeBytes(address+4, b"\x8d")
        #if item[0] == "Plasma Beam" :
        #    romWriter.writeBytes(address+4, b"\x8e")
        ## need to investigate
...
def Main(argv: list[str], romWriter: Optional[RomWriter] = None) -> None:
    ...
    random.seed(seeeed)
    rom_name = f"SZM{seeeed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/SZM.sfc"
    ...
    # Edit plmid for certain locations ##need to investigate
    #romWriter.writeBytes(0x313544, b"\x98")
    #romWriter.writeBytes(0x311392, b"\x99")
    #romWriter.writeBytes(0x311800, b"\x9a")

    #restore gravity pickup behavior (no bonus items!)
    romWriter.writeBytes(0x262ff, b"\x20\x00")

    #return backdoor xray to within the destroyed room so its accessible
    romWriter.writeBytes(0x3118a0, b"\x38\x1c")

    #hellrun speed echoes patch ##verify
...
def assumed_fill(game: Game, spoilerSave: str) -> tuple[bool, str]:
    ...
    while fill_algorithm.count_items_remaining():
        ...
        if fill_algorithm.count_items_remaining() == 0:
            ...
            ## use this for early morph
            #completable = (
            #    game.all_locations["Morph Ball"]["item"] == Items.Morph or
            #    game.all_locations["Retro Ceiling Energy Tank"]["item"] == Items.Morph
                #)
            completable = True  #let's say it is
